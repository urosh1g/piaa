...
void dijkstra(graph_t* graph, int start_node) {
...
void dot_print_node(node_t* node, FILE* sink) {
	fprintf(sink, "    \"%p\"[label=\"%d:%d\"];\n",
			(void*)node, 
			node->value,
			node->dist
	);
	edge_t* edge = node->adj;
	while(edge) {
		fprintf(sink, "    \"%p\" -> \"%p\"[label=\"%d\"];\n", (void*)node, (void*)edge->dest, edge->weight);
		edge = edge->next;
	}
}
...
void dot_print_graph(graph_t* graph, FILE* sink) {
	...
	fprintf(sink, "digraph G {\n");
	while(node) {
		dot_print_node(node, sink);
		node = node->next;
	}
	fprintf(sink, "}\n");
}
